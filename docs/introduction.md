---
title: Introduction
description: What aster is, what it is not, and why it exists as the public runx target.
---

# aster

`aster` is the public repo where `runx` is supposed to prove that governed
...
That claim only matters if the repo remains inspectable. `aster` should get
...
- the intention of `aster`
- the architecture and memory boundary between `aster` and `runx`
...
`aster` is succeeding when the public repo shows a believable trail of
bounded improvement:
