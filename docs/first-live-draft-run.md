...
- `automaton` is a valid live target for real `runx` executions today.
...
- add a scafld bootstrap path so `issue-to-pr` and governed patch lanes can run
  directly here
