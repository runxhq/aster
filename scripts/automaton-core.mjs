...
import { buildContextBundle, renderContextPrompt } from "./build-automaton-context.mjs";
import { buildPromotionDrafts, writePromotionDrafts } from "./promote-automaton-state.mjs";
...
async function main(argv = process.argv.slice(2)) {
  ...
  const outputs = await runAutomatonCore(options);
  process.stdout.write(`${JSON.stringify(outputs, null, 2)}\n`);
}

export async function runAutomatonCore(options) {
  if (!options.runxArgs?.length) {
    throw new Error("automaton-core requires a runx invocation after --.");
  }
